...
// Run: PLAYWRIGHT_BASE_URL=http://127.0.0.1:8000 npx playwright test e2e/full_simulation.spec.js --project=chromium
// Note: In the built Django-served SPA, BASE_URL may be '/', so use root baseURL (not /app/).
...
test.describe('Full simulation – Pharmacy (Register to Daily Report)', () => {
  test.beforeAll(() => {
    fs.mkdirSync(SCREENSHOT_DIR, { recursive: true })
    console.log('Screenshots will be saved to:', SCREENSHOT_DIR)
  })

  test('Register → Setup Wizard (pharmacy) → feature list → daily report (screenshots)', async ({ page }) => {
    const username = 'demo_' + String(Date.now())
    const email = `demo_${Date.now()}@e2e.local`
    ...
    // NOTE: In the built app, Register screen is username/password style.
    await expect(page.getByRole('heading', { name: /စာရင်းသွင်းရန်|register/i })).toBeVisible({ timeout: 10000 })
    await page.getByPlaceholder(/enter your username/i).fill(username)
    // Email or phone is required by backend validation; this UI only has email.
    ...
    if (page.url().includes('/register')) {
      ...
      if (err) {
        await page.getByPlaceholder(/enter your username/i).fill('demo_' + String(Date.now()) + '_2')
        await page.getByRole('button', { name: /စာရင်းသွင်းရန်|register/i }).click()
        await page.waitForTimeout(4000)
      }
    }

    // Some builds redirect to /login after successful registration (no auto-login).
